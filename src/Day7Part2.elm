module Day7Part2 exposing (main)
...
findDirectoryToDelete fileStructure =
    case fileStructure of
        Dir files ->
            let
                totalSizeUsed =
                ...
                totalSizeUnused =
                    70000000 - totalSizeUsed

                spaceNeeded =
                    30000000 - totalSizeUnused
            in
            fileStructure
                |> toDirectorySizes
                |> List.filter (\dirSize -> dirSize >= spaceNeeded)
                |> List.minimum

        File _ ->
            Debug.todo "findDirectoryToDelete"
