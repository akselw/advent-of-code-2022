module Day5Part2 exposing (main)
...
performSteps cratePlan =
    List.foldl performMultiStep cratePlan.startingPoint cratePlan.steps


performMultiStep : Step -> CrateState -> CrateState
performMultiStep step crateState =
    let
        elementsToMove : Maybe (List Char)
        elementsToMove =
            crateState
                |> Array.get (step.fromStack - 1)
                |> Maybe.map (List.take step.n)
    in
    elementsToMove
        |> Maybe.map (performMultiStepWithElements step crateState)
        |> Maybe.withDefault crateState


performMultiStepWithElements : Step -> CrateState -> List Char -> CrateState
performMultiStepWithElements step crateState char =
    crateState
        |> Array.indexedMap
            (\index stack ->
                if index == (step.fromStack - 1) then
                    List.drop step.n stack

                else if index == (step.toStack - 1) then
                    List.concat [ char, stack ]
